...
def view_problems(problem_brackets, content):
	...
	print "bracket problems to fix:"; print
...
def validate_structure(fn, verbose=False):
	...
	if verbose: print "structure validated successfully"
	return 1
...
def validate_content(fn, verbose=False):
	...
	if output_bracketless == True:
		out_fn = fn[:-4] + '_bracketless.txt'
		print 'outputting bracketless file: ', out_fn
		f2 = open(out_fn, 'w')
...
if __name__ == '__main__':
	...
	if len(sys.argv) == 1: print "argument needed"; exit()

	fn = sys.argv[1]
	if fn[-4:] != ".txt": print ".txt needed as first argument"; exit()

	if not ("-s" in sys.argv or "-c" in sys.argv): print "which mode? ( -s / -c )"; exit()
